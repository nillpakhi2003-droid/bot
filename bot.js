...
async function lockQuestion(qid) {
  ...
  try {
    const response = await fetch(config.LOCK_URL, {
      method: 'POST',
      headers: {
        'X-Appwrite-Project': '643c467a7dbb0655970d',
  ...
  while (true) {
    ...
    try {
      ...
      const accountRes = await fetch('https://server.acsdoubts.com/v1/account', {
        headers: { 'X-Appwrite-Project': '643c467a7dbb0655970d' },
        credentials: 'include'
      });
      ...
      const url = `${config.CHECK_URL_BASE}?queries[0]=equal("lockedBy",["${userId}"])`;
...
async function main() {
  console.log('╔═══════════════════════════════════════════╗');
  console.log('║  🤖 Fast WebSocket Auto-Lock Bot 🤖     ║');
  console.log('╚═══════════════════════════════════════════╝\n');
  
  console.log('⚠️  IMPORTANT: Login to https://acsdoubts.com in your browser first!');
  console.log('⚠️  This bot uses your browser cookies automatically.\n');
...
async function launchBrowser() {
  console.log('🚀 Launching your Firefox with existing session...');
  
  // Launch Firefox with persistent context (uses your existing profile and cookies)
  const launchOptions = {
  ...
  const context = await firefox.launchPersistentContext(config.FIREFOX_PROFILE, launchOptions);
  ...
  // Navigate to the website (uses your existing cookies and session)
  console.log(`🌐 Navigating to ${config.WEBSITE_URL}...`);
  await page.goto(config.WEBSITE_URL, { waitUntil: 'domcontentloaded' });
  
  console.log('✅ Browser ready with your existing session!');
